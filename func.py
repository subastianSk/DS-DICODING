...
class BrazilMapPlotter:
    ...
    def calculate_conversion_rate(self):
        # Menghitung jumlah kunjungan atau tampilan halaman produk
        page_views = len(self.df)

        # Menghitung jumlah pesanan yang berhasil dibuat
        successful_orders = len(self.df[self.df['order_status'] == 'delivered'])

        # Menghitung rasio konversi
        conversion_rate = (successful_orders / page_views) * 100

        return page_views, successful_orders, conversion_rate

    def calculate_returned_and_cancelled_products(self):
        # Menghitung jumlah pengembalian untuk setiap kategori produk
        returned_products = self.df[self.df['order_status'] == 'delivered']  # Ubah 'returned' menjadi 'delivered' untuk menghitung pengembalian
        returned_product_counts = returned_products['product_category_name_english'].value_counts().head(10)

        # Menghitung jumlah pembatalan untuk setiap kategori produk
        cancelled_products = self.df[self.df['order_status'] == 'canceled']
        cancelled_product_counts = cancelled_products['product_category_name_english'].value_counts().head(10)

        return returned_product_counts, cancelled_product_counts